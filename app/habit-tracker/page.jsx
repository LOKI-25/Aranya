...
export default function HabitTrackerPage() {
  ...
  // Update the fetchHabitTracking function to store tracking IDs
  ...
  // Update the fetchHabits function to store tracking IDs
  const fetchHabits = async () => {
    ...
    try {
      ...
      const trackingIdsMap = {} // Map to store tracking IDs by date and habit ID
      ...
      trackingData.forEach((entry) => {
        ...
        const user = entry.user // Get user from tracking entry instead of habit
        ...
        if (!habitMap.has(habitId)) {
          habitMap.set(habitId, {
            ...
            user: user, // Use user from tracking entry
        ...
        trackingIdsMap[date][habitId] = trackingId
      })
      ...
      setTrackingIds(trackingIdsMap) // Store tracking IDs
  ...
  // Update the toggleCompletion function to use tracking IDs
  const toggleCompletion = async (habitId, dayKey) => {
    setIsSubmitting(true)
    ...
    try {
      ...
      // Get the tracking ID for this habit and day
      const trackingId = trackingIds[dayKey]?.[habitId]

      // If we have a tracking ID, update the existing record
      if (trackingId) {
        // Update completion status via API
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/habit_tracking/${trackingId}/`, {
          method: "PUT",
          headers,
          body: JSON.stringify({
            date: dayKey,
        ...
        // Show success toast
        toast({
          title: isCurrentlyCompleted ? "Habit unmarked" : "Habit completed",
          description: isCurrentlyCompleted
            ? "Habit has been marked as not completed."
            : "Habit has been marked as completed. Keep up the good work!",
        })
      } else {
        // If no tracking ID exists, create a new tracking record
        const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/habit_tracking/`, {
          method: "POST",
          headers,
          body: JSON.stringify({
        ...
        // Get the new tracking ID from the response
        const data = await response.json()

        // Update tracking IDs map
        setTrackingIds((prev) => {
          const newTrackingIds = { ...prev }
          if (!newTrackingIds[dayKey]) {
            newTrackingIds[dayKey] = {}
          }
          newTrackingIds[dayKey][habitId] = data.id
          return newTrackingIds
        })

        // Show success toast
        toast({
          title: "Habit tracking started",
          description: "You've started tracking this habit. Keep up the good work!",
        })
      }
                        ...
                        {weekDays.map((day) => {
                          ...
                          return (
                            <button
                              ...
                              onClick={() => toggleCompletion(habit.id, day.dayKey)}
                              disabled={isSubmitting}
